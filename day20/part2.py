...
def manhattanDistanceCells(grid, grid_size, start, distance):
    """
    Finds the coordinates of all cells within a Manhattan distance of `distance` or less.

    :param grid_size: Tuple (rows, cols) defining the grid dimensions.
                      Pass None for an unbounded grid.
    :param start: Tuple (x, y) for the starting point.
    :param distance: The Manhattan distance limit.
    :return: A set of coordinates (x, y) within the given Manhattan distance.
    """
    ...
    for dx in range(-distance, distance + 1):
        for dy in range(-distance + abs(dx), distance - abs(dx) + 1):
            ...
            # If grid size is provided, ensure the coordinates are within bounds
            rows, cols = grid_size
            if 0 <= x < rows and 0 <= y < cols and grid[x][y] != '#':
                reachable_coords.add((x, y, abs(dx) + abs(dy)))
...
for row, line in enumerate(grid):
    for col, cell in enumerate(line):
        adjList = manhattanDistanceCells(grid, (len(grid), len(grid[0])), (row, col), 1)
        if cell != '#':
            for adjRow, adjCol, dist in manhattanDistanceCells(grid, (len(grid), len(grid[0])), (row, col), 1):
                graph.add_edge((row, col), (adjRow, adjCol))
            if cell == 'S':
                startPos = (row, col)
            if cell == 'E':
                endPos = (row, col)
            
            for adjRow, adjCol, dist in manhattanDistanceCells(grid, (len(grid), len(grid[0])), (row, col), 20):
                shortCuts.append(((row, col), (adjRow, adjCol), dist))
...
for start, end, dist in shortCuts:
    shortCutSavings = minLegalDist - (shortestDistancesFromStart[start] + shortestDistancesFromEnd[end] + dist)
